      ...
      <div class="flex row justify-b">
        <div class="flex flex-col gap-2 grow py-4 px-5 bg-white">
          ...
          <div class="flex grow-x bg-white border border-[#8c8c8c] rounded-lg py-2 items-center">
            ...
            class="idBarang" 
            value={{ $dataBarang->id }}>
                <span class="w-full outline-none text-sm bg-transparent px-2">{{ $dataBarang->nama }}</span>
          </div>         
          ...
          <label class="text-black w-[25%]">Berat Akhir</label>
            <input class="beratAkhir w-full border-b border-[#8c8c8c] focus:outline-none flex items-center h-full text-sm bg-transparent px-2" 
                  type="number"
                  onchange="document.getElementById('tombol-produksi').disabled = false"
                  required> gr
        </div>
        </div>
        ...
        bg-gray-100 border-t text-start" id="bahan{{ $index+1 }}">
            ...
            <div 
              ...
              id="stock{{ $index+1 }}"  
            >
            {{ $resep->bahan->jumlah }} {{ $resep->bahan->satuan }}
            </div>
            ...
            <div class="w-[25%] text-center text-[#635549]">
              <input 
                type="checkbox" 
                class="pakaiStock" 
                onchange="cekStock({{ $index+1 }}})"
                {{$resep->bahan->jumlah <= 0 ? 'disabled' : ''}}
              />
            </div>
            ...
            <div class="w-[15%] text-[#635549] flex-row">
              ...
              >
                ...
                <input 
                  ...
                  type="text"
                  id="hargaBahan${i}"
                  value="{{ $resep->takaran*$resep->bahan->harga }}"
                  oninput="formatRupiah(this,{{ $index+1 }})"
                />
                <input 
                  class="bahanHarga"
                  type="hidden"
        ...
        <button
          ...
          id="tombol-produksi"
          class="inline-flex w-[25%] justify-center rounded-md bg-gradient-to-b from-[#B175FB] to-[#001476] px-3 py-2 mx-3 my-2 text-sm font-semibold text-white 
          hover:opacity-50
...
<script>
  ...
  {
    e.preventDefault();
    document.getElementById('tombol-produksi').disabled = true;
